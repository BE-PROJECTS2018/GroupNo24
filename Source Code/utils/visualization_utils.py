...
def visualize_only_mapped_boxes_on_image_array(
    image,
	index,
    boxes,
    classes,
  ...
  for i in range(boxes.shape[0]):
    if (classes[i] == index) and (scores is None or scores[i] > 0.5):
      box = tuple(boxes[i].tolist())
      if scores is None:
        box_to_color_map[box] = groundtruth_box_visualization_color
      else:
  ...
  # Draw mapped boxes onto image (max 5).
  centX = [0]*5
  centY = [0]*5
  k = 0
  for box, color in box_to_color_map.items():
    ymin, xmin, ymax, xmax = box
    centX[k] = int(((xmax+xmin)*400)/2)
    centY[k] = int(((ymax+ymin)*300)/2)
    k+=1
    draw_bounding_box_on_image_array(
        image,
        ymin,
...
def objects_in_frame(scores, classes):
	new_classes = {}
	k = 0
	for i in range(scores.shape[0]):
		if scores[i]>0.5:
			new_classes[k] = classes[i]
			k+=1
	return new_classes